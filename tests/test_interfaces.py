...
from notesvault.terminal import TerminalCLI
from devtools.synthetic import SyntheticAuthentication, SyntheticNotesProvider


def terminal(tmp_path, authentication=None):
    store = ConfigStoreController(tmp_path / "settings")
    if authentication is None:
        authentication = SyntheticAuthentication(store)
        authentication.connected = True
        authentication.account = "demo"
        app = Application(store, authentication=authentication, provider_factory=SyntheticNotesProvider)
    else:
        app = Application(store, authentication=authentication)
    return TerminalCLI(app, Console(file=StringIO(), width=100))
...
def test_terminal_saved_login_verification_retry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("notesvault.terminal.Prompt.ask", lambda *a, **kw: next(codes))
...
def test_terminal_cancel_clears_pending_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("notesvault.terminal.Prompt.ask", interrupt)
...
def test_terminal_cancel_waits_for_worker_and_releases_task(tmp_path, monkeypatch):
    ...
    sleep = time.sleep
    def interrupt(delay):
        if current_thread() is main_thread() and not ui.quitting:
            assert started.wait(3)
            raise KeyboardInterrupt()
        sleep(delay)
    monkeypatch.setattr("notesvault.terminal.time.sleep", interrupt)
    ui.fetch()
    assert ui.quitting
    assert not ui.tasks.busy
    assert "cancelled" in ui.console.file.getvalue()
    ui.tasks.close()
...
def test_terminal_saved_login_failure_uses_masked_password(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("notesvault.terminal.Prompt.ask", prompt)
    assert ui.login()
...
def test_tui_import_does_not_import_qt():
    result = subprocess.run([sys.executable, "-c", "import notesvault.terminal; import sys; assert not any(k.startswith(('edifice', 'PySide6')) for k in sys.modules)"], capture_output=True, text=True, timeout=15)
    assert result.returncode == 0, result.stderr


def test_tui_rejects_redirected_input():
    result = subprocess.run([sys.executable, "-m", "notesvault", "--tui"], input="", capture_output=True, text=True, timeout=15)
